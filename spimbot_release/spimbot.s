...
puzzle_for:
    li $t1, 40
    bge $s1, $t1, puzzle_end
    li $s2, 0                                   #   $s2 = puzzle_received = 0
    la $s3, puzzlewrapper                       #   $s3 = puzzle
    sw $s3, REQUEST_PUZZLE            

    puzzle_while:
        bne $s2, 0, puzzle_next
        j puzzle_while
    
    puzzle_next:

        # get arguments for solve_queens

        lw  $t2, 0($s3)                         #   $t2 = board_size
        lw  $t3, 4($s3)                         #   $t3 = **board
        lw  $t4, 8($s3)                         #   $t4 = given_queens     
        lw  $t5, 12($s3)                        #   $t5 = queens_to_place

        move $a0, $t3                           #   $a0 = **board
        move $a1, $t2                           #   $a1 = board_size
        move $a2, $t4                           #   $a2 = given_queens
        move $a3, $t5                           #   $a3 = queens_to_place
